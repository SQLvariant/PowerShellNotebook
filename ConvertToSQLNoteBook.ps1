...
if($ExtractInlineComments){
    $ScriptFrags = (Get-ParsedSql -ScriptPath $ScriptPath).ScriptTokenStream.where({$_.TokenType -in 'MultilineComment', 'SingleLineComment'})
}
else{
